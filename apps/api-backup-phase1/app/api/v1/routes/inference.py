...
def run_inference(
    ...
    if model.current_version_id:
        active_version = (
            ...
            .filter(ModelVersion.id == model.current_version_id, ModelVersion.model_id == model.id)
            .first()
        )
    output = run_mock_inference(model, active_version, payload.input)
    ...
    return InferenceResponse(
        ...
        output=output,
        latency_ms=latency_ms,
        usage={"input_units": input_units, "output_units": output_units},
    )
